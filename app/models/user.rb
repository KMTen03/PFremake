class User < ApplicationRecord
  ...
  def guest_user?
    email == "guest@example.com" # メールアドレスが完全一致するとき
  end

  def self.ranking
    find(UserPost.group(:user_id).order('count(post_id) desc').limit(5).pluck(:user_id))
  ...
  def get_profile_image(width, height)
    unless profile_image.attached? # プロフィールイメージが適用されていなければ
